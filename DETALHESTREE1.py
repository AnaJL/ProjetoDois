...
class No:
    ...
    def balanco(self):
        prof_esq = 0
        if self._esquerda:
            prof_esq = self._esquerda.altura()
        prof_dir = 0
        if self._direita:
            prof_dir = self._direita.altura()
    ...
    def altura(self):
        if self._dado == None:
            ...
            return 1 + max(alt_esq, alt_dir) #Pega a maior entre as duas pq será a altura

    def rotacaoEsquerda(self):
        p = self._dado
        q = self._id
        p , q , self._direita._dado, self._direita._id = self._direita._dado, self._direita._id, p , q
        EsquerdaAntiga = self._esquerda
        self.setaFilhos(self._direita, self._direita._direita)
        self._esquerda.setaFilhos(EsquerdaAntiga, self._esquerda._esquerda)

    def rotacaoDireita(self):
        p = self._dado
        q = self._id
        p, q, self._esquerda._dado, self._esquerda._id = self._esquerda._dado, self._esquerda._id, p, q
        DireitaAntiga = self._direita
        self.setaFilhos(self._esquerda._esquerda, self._esquerda)
        self._direita.setaFilhos(self._direita._direita, DireitaAntiga)
